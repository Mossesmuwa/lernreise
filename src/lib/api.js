...
export async function getLevels() {
  const { data, error } = await supabase
    .from('levels')
    .select('id, name, status, sort_order, courses(id, title, status, cover_image_url)')
    .order('sort_order');
...
export async function getCurrentCourse() {
  const { data, error } = await supabase
    .from('courses')
    .select('id, title, status, cover_image_url, level:levels(id, name), modules(id, name, sort_order, lessons(id, name, sort_order, status))')
    .eq('status', 'current')
    .maybeSingle();
  if (error) throw error;
  return data;
...
export async function getLessonDetail(lessonId) {
  const [{ data: lesson, error: e1 }, { data: sessions, error: e2 }, { data: classes, error: e3 }] = await Promise.all([
    supabase.from('lessons').select('id, name, status, started_at, completed_at').eq('id', lessonId).single(),
    supabase
      .from('study_sessions')
    ...
    supabase
      ...
      .select('id, scheduled_at, status, duration_minutes, teacher:teachers(name)')
      .eq('lesson_id', lessonId)
      .is('deleted_at', null)
...
export async function markLevelComplete(levelId, nextLevelId) {
  const { error: e1 } = await supabase.from('levels').update({ status: 'completed' }).eq('id', levelId);
  if (e1) throw e1;
  if (nextLevelId) {
    const { error: e2 } = await supabase.from('levels').update({ status: 'current' }).eq('id', nextLevelId);
    if (e2) throw e2;
  }
}

export async function setLevelCurrent(levelId) {
  const { error } = await supabase.from('levels').update({ status: 'current' }).eq('id', levelId);
  if (error) throw error;
}
...
export async function listStudySessions({ from, to, search } = {}) {
  let query = supabase
    .from('study_sessions')
    .select('id, session_date, duration_minutes, notes, lesson:lessons(id, name)')
    .is('deleted_at', null)
    .order('session_date', { ascending: false });
...
export async function addStudySession({ lessonId, sessionDate, durationMinutes, notes }) {
  const { error } = await supabase
    .from('study_sessions')
    .insert({ lesson_id: lessonId, session_date: sessionDate, duration_minutes: durationMinutes, notes });
  if (error) throw error;
}
...
export async function softDeleteStudySession(id) {
  const { error } = await supabase.from('study_sessions').update({ deleted_at: new Date().toISOString() }).eq('id', id);
  if (error) throw error;
}

export async function restoreStudySession(id) {
  const { error } = await supabase.from('study_sessions').update({ deleted_at: null }).eq('id', id);
  if (error) throw error;
}
...
export async function listTeacherClasses({ from, to } = {}) {
  let query = supabase
    .from('teacher_classes')
    .select('id, scheduled_at, original_scheduled_at, mode, status, duration_minutes, notes, lesson:lessons(id, name), teacher:teachers(id, name)')
    .is('deleted_at', null)
    .order('scheduled_at');
...
export async function addTeacherClass({ lessonId, teacherId, scheduledAt, mode, notes }) {
  const { error } = await supabase
    .from('teacher_classes')
    .insert({ lesson_id: lessonId, teacher_id: teacherId, scheduled_at: scheduledAt, mode, notes });
  if (error) throw error;
}

export async function rescheduleTeacherClass(id, newScheduledAt, currentScheduledAt) {
  const { error } = await supabase
    .from('teacher_classes')
    .update({
...
export async function softDeleteTeacherClass(id) {
  const { error } = await supabase.from('teacher_classes').update({ deleted_at: new Date().toISOString() }).eq('id', id);
  if (error) throw error;
}
...
export async function getActiveTimer() {
  const { data, error } = await supabase.from('active_timer').select('lesson_id, started_at').maybeSingle();
  if (error) throw error;
  return data;
}
...
export async function startTimer(lessonId) {
  ...
  const { error } = await supabase
    .from('active_timer')
    .upsert({ owner_id: userData.user.id, lesson_id: lessonId, started_at: new Date().toISOString() }, { onConflict: 'owner_id' });
  if (error) throw error;
}
...
export async function updateSettings(patch) {
  const { data: userData } = await supabase.auth.getUser();
  const { error } = await supabase.from('settings').upsert({ owner_id: userData.user.id, ...patch }, { onConflict: 'owner_id' });
  if (error) throw error;
}

export async function getPublicProfile() {
  const { data, error } = await supabase.from('public_profile').select('display_name, avatar_url').maybeSingle();
  if (error) throw error;
  return data;
}
...
export async function updatePublicProfile(patch) {
  ...
  const { error } = await supabase
    .from('public_profile')
    .upsert({ id: true, owner_id: userData.user.id, ...patch }, { onConflict: 'id' });
  if (error) throw error;
}
...
export async function listShareLinks() {
  const { data, error } = await supabase
    .from('share_links')
    .select('id, role, teacher_id, teacher:teachers(name), token, code, label, expires_at, revoked, created_at')
    .order('created_at', { ascending: false });
  if (error) throw error;
  return data;
}

export async function createShareLink({ role, teacherId, label, expiresAt, withCode }) {
...
export async function recordShareAccess(shareLinkId) {
  const { error } = await supabase.rpc('record_share_access', { p_share_link_id: shareLinkId });
  if (error) throw error;
}
...
export async function getTeacherClassesForToken(token) {
  const { data, error } = await supabase.rpc('get_teacher_classes', { p_token: token });
  if (error) throw error;
  return data;
}
