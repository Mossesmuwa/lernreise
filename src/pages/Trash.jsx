...
import { listDeletedStudySessions, listDeletedTeacherClasses, restoreStudySession, restoreTeacherClass } from '../lib/api';
  ...
  return (
    <div className="p-4 md:p-0 max-w-md md:max-w-none mx-auto space-y-5">
      ...
      <p className="text-xs text-ink/60">
        Deleted study sessions and classes stay here until restored — nothing is ever removed for good from here.
      </p>

      {empty && <p className="text-sm text-ink/40">Nothing in the trash.</p>}

      {sessions.map((s) => (
        <div key={s.id} className="flex justify-between items-center bg-card border border-mist rounded-lg px-4 py-2.5">
      ...
      {classes.map((c) => (
        <div key={c.id} className="flex justify-between items-center bg-card border border-mist rounded-lg px-4 py-2.5">
          <div className="text-sm">
