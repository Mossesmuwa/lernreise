...
export default function Login() {
  ...
  const [lang] = useState('de'); // will read from settings once authenticated elsewhere
      ...
      <div className="w-full max-w-sm bg-card border border-mist rounded-2xl p-8">
        ...
        <div className="w-16 h-16 rounded-full bg-paper border border-mist flex items-center justify-center overflow-hidden mb-4 mx-auto">
          {profile?.avatar_url ? (
            ...
            <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="text-mist">
        ...
        <p className="text-sm text-ink/60 text-center mb-6">{t('signInSubtitle', lang)}</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label htmlFor="password" className="block text-xs text-ink/60 mb-1">
              {t('password', lang)}
            </label>
            <input
              id="password"
