...
export default function CalendarPage() {
  ...
  async function load() {
    const [cls, tchrs, course] = await Promise.all([
      listTeacherClasses({ from: weekStart.toISOString(), to: weekEnd.toISOString() }),
      listTeachers(),
      getCurrentCourse(),
    ]);
    setClasses(cls);
  ...
  return (
    <div className="p-4 md:p-0 max-w-4xl mx-auto space-y-5">
        ...
        {days.map((d, i) => {
          ...
          return (
            <div key={i}>
              ...
              <div className={`text-sm py-1.5 rounded-full ${isToday ? 'bg-pine-soft text-pine-deep' : ''}`}>
                {d.getDate()}
              </div>
              {hasClass && <div className="w-1 h-1 rounded-full bg-pine mx-auto mt-0.5" />}
            </div>
          );
        })}
      </div>
      ...
      <div>
        ...
        {classes.length === 0 && <p className="text-sm text-ink/40">Nothing scheduled.</p>}
        {classes.map((c) => (
          <button
            key={c.id}
          ...
          >
            <span>
              <span className="mr-2">
                {new Date(c.scheduled_at).toLocaleDateString([], { weekday: 'short', day: 'numeric' })} ·{' '}
